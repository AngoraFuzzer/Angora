...
*Some projects also requires the `LD` to be set to `angora-clang` for compilation.*
...
The running result of Angora that use wllvm/gllvm may be different from compiling directly because compiler optimization.

## Build external libraries
if the external libraries are your targets (e.g. you are interesting and want solve constraints in them),
we can use `USE_TRACK=1 make` to compile it.
Otherwise, we can use following rules to propagate taints while our program enter their code.
Here, we use zlib as example.

###  Model external library
View it as a blockbox, and defined models for them. See rules in https://clang.llvm.org/docs/DataFlowSanitizer.html.
- ignore this libary
...
# or set all as custom (do not recommend)
./angora/tools/gen_library_abilist.sh /usr/lib/x86_64-linux-gnu/libz.so  custom > zlib_abilist.txt

export ANGORA_TAINT_RULE_LIST=/path-to/zlib_abilist.txt
# write your custom fucntion, e.g. llvm_mode/external_lib/zlib-func.c
# compile it and 
...
## Build C++ program and C++ standard libray
- C++ program: CXX=/path-to-angora/bin/angora-clang++ or -DCMAKE_CXX_COMPILER=...
- C++ standard library: we have built one under ubuntu 16.04 64bits in llvm_mode/libcxx_dfsan. You can built it by yourself following commands and move the libraries to llvm_mode/libcxx_dfsan directory and bin/. (run libcxx_dfsan/compile.sh)
