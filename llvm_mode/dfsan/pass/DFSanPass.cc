...
struct DFSanFunction {
  ...
  DFSanFunction(DataFlowSanitizer &DFS, Function *F, bool IsNativeABI)
      : DFS(DFS), F(F), IA(DFS.getInstrumentedABI()), IsNativeABI(IsNativeABI),
        ArgTLSPtr(nullptr), RetvalTLSPtr(nullptr), LabelReturnAlloca(nullptr) {
    DT.recalculate(*F);
...
FunctionType *DataFlowSanitizer::getCustomFunctionType(FunctionType *T) {
  ...
  for (FunctionType::param_iterator i = T->param_begin(), e = T->param_end();
    ...
    if (isa<PointerType>(*i) &&
        (FT =
             dyn_cast<FunctionType>(cast<PointerType>(*i)->getElementType()))) {
  ...
  if (GetArgTLSPtr) {
    ...
    GetArgTLS = ConstantExpr::getIntToPtr(
        ...
        PointerType::getUnqual(FunctionType::get(
            PointerType::getUnqual(ArgTLSTy), (Type *)nullptr)));
  }
  if (GetRetvalTLSPtr) {
    RetvalTLS = nullptr;
    GetRetvalTLS = ConstantExpr::getIntToPtr(
        ...
        PointerType::getUnqual(FunctionType::get(
            PointerType::getUnqual(ShadowTy), (Type *)nullptr)));
  }
...
Constant *DataFlowSanitizer::getOrBuildTrampolineFunction(FunctionType *FT,
  ...
  if (F && F->isDeclaration()) {
    ...
    for (unsigned N = FT->getNumParams(); N != 0; ++AI, --N)
      Args.push_back(&*AI);
    CallInst *CI =
        CallInst::Create(F->arg_begin(), Args, "", BB);
    ReturnInst *RI;
    if (FT->getReturnType()->isVoidTy())
...
bool DataFlowSanitizer::runOnModule(Module &M) {
  ...
  for (Module::alias_iterator i = M.alias_begin(), e = M.alias_end(); i != e;) {
   ...
   ReadOnlyNoneAttrs.addAttribute(Attribute::ReadOnly).addAttribute(Attribute::ReadNone);
...
void DFSanVisitor::visitCallSite(CallSite CS) {
  ...
  if (i != DFSF.DFS.UnwrappedFnMap.end()) {
    Function *F = i->second;
    // errs() << "KIND " <<  DFSF.DFS.getWrapperKind(F) << "\n";
      ...
      if (CallInst *CI = dyn_cast<CallInst>(CS.getInstruction())) {
        ...
        if (FT->isVarArg()) {
          ...
          auto *LabelVAAlloca = new AllocaInst(
              LabelVATy,
              DFSF.F->getParent()->getDataLayout().getAllocaAddrSpace(),
              "labelva",
