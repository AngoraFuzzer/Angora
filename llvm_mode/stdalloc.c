...
__attribute__((visibility("default"))) void 
__dfsw_free(void * ptr, dfsan_label ptr_label) {
...
__attribute__((visibility("default"))) void *
__dfsw_calloc(size_t nmemb, size_t size, 
        dfsan_label nmemb_label, dfsan_label size_label, dfsan_label *ret_label) {
...
//  DEBUG_PRINTF("[+] Old metadata retrieved as: base = %p, size = %lu\n", old_base, old_size);
  ...
  if (ret) {
    if (ptr && ret != ptr) {
      ...
      heapmap_invalidate(ptr);
#endif
      dfsan_label * old_label_area = dfsan_shadow_for(ptr);
